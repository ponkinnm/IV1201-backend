...
export class ApplicationController {
   ...
   */
  getAllApplications: RequestHandler = async (
    req: Request,
    res: Response,
    next: NextFunction
  ): Promise<void> => {
   ...
   */
  getApplicationDetailsById: RequestHandler = async (
    req: Request,
    res: Response,
    next: NextFunction
  ): Promise<void> => {
    try {
      ...
      const applicationDetail = await sequelize.transaction(async () => {
        return await this.applicationService.getApplicationDetailsById(
          application_id
        );
      });
      ...
      if (!applicationDetail) {
        ...
        !(
          AuthService.isRecruiter(req.user) ||
          applicationDetail.person.person_id === req.user!.person_id
        )
      ) {
        res.status(401).json({ error: 'Unauthorized' });
      } else {
   ...
   */
  updateApplicationStatus: RequestHandler = async (
    req: Request,
    res: Response,
    next: NextFunction
  ): Promise<void> => {
   ...
   */
  submitApplication: RequestHandler = async (
    req: Request,
    res: Response,
    next: NextFunction
  ): Promise<void> => {
