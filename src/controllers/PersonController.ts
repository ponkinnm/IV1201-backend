...
export class PersonController{
     ...
     */
    addNewUser: RequestHandler = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
        try{
            const {name, surname, pnr, email, username, password, role_id } = req.body;
            const addedUser = await sequelize.transaction( async () => {
                 return await this.personService.addNewUser(name, surname, pnr, email, username, password, role_id);
                });
     ...
     */
    UpdateUserNameAndPassword: RequestHandler = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
        try{
            const {person_id, username, password} = req.body;
            const updatedUser = await sequelize.transaction(async()=> {
                return await this.personService.addUsernameAndPassword(person_id, username, password);
            })
     ...
     */
    updateEmail: RequestHandler = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
        try{
     ...
     */
    getUserDetailsById : RequestHandler = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
        try{
