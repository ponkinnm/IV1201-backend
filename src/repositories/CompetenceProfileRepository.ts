...
export class CompetenceProfileRepository
  implements ICompetenceProfileRepository
{
  /**
  ...
  async getCompetenceProfileById(person_id: number) {
    ...
    const competences = await CompetenceProfile.findAll({
      where: { person_id },
      include: [
        {
          model: Competence,
    ...
    const competenceProfileDTO: CompetenceProfileDTO[] = competences.map(
      (app) =>
        new CompetenceProfileDTO(app.Competence!.name, app.years_of_experience)
    );
  ...
  async addNewCompetenceProfile(
    ...
    try {
      for (const competence of competences) {
        ...
        Validators.isPositiveNumber(competence.years_of_experience);
      }
